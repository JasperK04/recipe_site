...
def _request_reset(client, identifier, monkeypatch):
    ...
    response = client.post(
        "/api/password-reset/request", json={"identifier": identifier}
    )
...
def test_reset_request_is_enumeration_safe_and_accepts_username_or_email(
    ...
    with app.app_context():
        credential = (
            Credential.query.filter_by(purpose=PASSWORD_RESET_PURPOSE)
...
def test_reset_verification_and_completion_are_single_use(
    app, client, user, monkeypatch
):
    ...
    assert client.post(
        "/api/password-reset/verify", json={"token": token}
    ).get_json() == {"valid": True}
...
def test_reset_rejects_bad_password_and_does_not_consume_token(
    app, client, user, monkeypatch
):
    _, sent = _request_reset(client, "chef", monkeypatch)
    token = sent[0][1]
...
def test_reset_page_shows_email_without_token_field(client, user, monkeypatch):
    _, sent = _request_reset(client, "chef", monkeypatch)
    response = client.get(f"/account/wachtwoord-resetten?token={sent[0][1]}")
...
def test_reset_page_uses_server_side_credential_on_post(client, user, monkeypatch):
    _, sent = _request_reset(client, "chef", monkeypatch)
    client.get(f"/account/wachtwoord-resetten?token={sent[0][1]}")
    ...
    assert response.status_code == 302
    assert response.headers["Location"].endswith("/account/inloggen")
